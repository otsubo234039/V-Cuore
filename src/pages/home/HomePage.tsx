    ...
    <>
      <style dangerouslySetInnerHTML={{
        ...
        @media (max-height: 880px) {
          .home-system-label { display: none; }
        }
      `}} />
      ...
      <div className="fixed inset-0 w-full h-full bg-main-bg text-oshi-primary font-sans flex flex-col items-center transition-colors duration-500 overflow-hidden">
        ...
        <div className="relative w-full flex-1 overflow-hidden">
          ...
          <main className="no-scrollbar relative z-10 w-full h-full flex flex-col items-center justify-center px-[2.5%] py-2 md:py-3 gap-4 md:gap-6 overflow-hidden select-none">
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-[4%] transition-all duration-700 ${viewMode === 'mobile' ? 'w-[85%]' : 'w-[70%] max-w-850px'}`}>
              <div className="bg-card-bg px-6 py-4 md:px-7 md:py-5 rounded-3xl border-2 border-oshi-primary/20 dark:border-oshi-primary/40 transition-colors duration-500">
            ...
            <div className={`bg-card-bg backdrop-blur-xl border-2 border-oshi-primary/10 dark:border-oshi-primary/20 px-6 py-5 md:px-7 md:py-6 rounded-3xl transition-colors duration-700 ${viewMode === 'mobile' ? 'w-[85%]' : 'w-[70%] max-w-850px'}`}>
              <h2 className="text-[11px] font-black uppercase tracking-[0.4em] border-l-4 border-oshi-primary pl-4 mb-4">Weakness Top 3</h2>
              <div className="flex flex-col gap-3">
                {weaknessItems.map((item, i) => (
                  <button
                    ...
                    className="flex justify-between items-center py-3 px-5 md:py-4 md:px-6 rounded-2xl border-2 border-oshi-primary/30 dark:border-oshi-primary bg-card-bg text-oshi-primary transition-all cursor-pointer group hover:bg-oshi-primary/5 text-left"
                  >
            ...
            <div className="flex flex-col items-center gap-3 py-1">
              <button
                ...
                className="bg-card-bg text-oshi-primary border-2 border-oshi-primary font-black py-3 px-14 md:px-20 rounded-full text-base md:text-lg transition-all duration-300 cursor-pointer outline-none shadow-none hover:bg-oshi-primary hover:text-white dark:hover:text-black"
              >
                学習を開始する
              </button>
        ...
        <div className="home-system-label fixed bottom-[3%] right-[4%] text-[9px] opacity-20 font-mono font-bold uppercase tracking-widest pointer-events-none transition-colors">
