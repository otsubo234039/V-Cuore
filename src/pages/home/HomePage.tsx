...
function HomePage({ isLoggedIn, onLoginClick, onSettingsClick, onStartClick }: HomePageProps) {
  ...
  return (
    <>
      ...
      {/* 🚀 コンテナ：bg-main-bg により、ライトは白、ダークは漆黒へと自動遷移 */}
      <div className="fixed inset-0 w-full h-full bg-main-bg text-oshi-primary font-sans flex flex-col items-center transition-colors duration-500 overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-65 dark:opacity-75 filter saturate-125 contrast-110 dark:saturate-150 dark:contrast-125 transition-all duration-500">
          <div className={`relative w-full h-full mx-auto transition-all duration-1000 ${viewMode === 'web' ? 'w-[95%] max-w-1440px aspect-video' : 'w-[95%] aspect-9/16'
            ...
            {[...Array(6)].map((_, i) => (
              <div key={i} className="absolute w-[2%] h-[3%] bg-oshi-primary rounded-full animate-leaf blur-[1px] opacity-45"
        ...
        <main className="no-scrollbar relative z-10 w-full flex-1 flex flex-col items-center justify-center p-[2%] md:p-[4%] gap-[6%] overflow-y-auto select-none">
          ...
          {/* 🚀 上部カード：背景を環境に完全に馴染ませ、枠線の「色」で存在を定義 */}
          ...
          {/* 🛰️ 下部リスト：2枚目の画像通り、アイテムごとに「枠」をクッキリさせる */}
          <div className={`bg-card-bg backdrop-blur-xl border-2 border-oshi-primary/10 dark:border-oshi-primary/20 p-[5%] rounded-3xl transition-colors duration-700 ${viewMode === 'mobile' ? 'w-[85%]' : 'w-[70%] max-w-850px'}`}>
            ...
            <div className="flex flex-col gap-4">
              {["IAM ポリシーの優先順位", "S3 バケットポリシーの記述", "VPC エンドポイントの構成"].map((text, i) => (
                <div key={i} className="flex justify-between items-center p-[4%] px-[6%] rounded-2xl border-2 border-oshi-primary/30 dark:border-oshi-primary bg-card-bg text-oshi-primary transition-all cursor-pointer group hover:bg-oshi-primary/5">
                  <span className="text-xs sm:text-sm font-black tracking-tight">{i + 1}. {text}</span>
        ...
        bg-card-bg 
        text-oshi-primary 
        border-2 border-oshi-primary 
        font-black py-4 px-20 rounded-full text-lg 
        transition-all duration-300 cursor-pointer outline-none shadow-none
        hover:bg-oshi-primary 
